...
const MANAGER_TOOLS  = new Set(["close_position", "claim_fees", "claim_all_rewards", "swap_token", "update_config", "get_position_pnl", "get_my_positions", "set_position_note", "add_pool_note", "get_wallet_balance", "get_swap_quote"]);
const SCREENER_TOOLS = new Set(["deploy_position", "get_active_bin", "get_top_candidates", "check_smart_wallets_on_pool", "get_token_holders", "get_token_narrative", "get_token_info", "search_pools", "get_pool_memory", "add_pool_note", "add_to_blacklist", "update_config", "get_wallet_balance", "get_my_positions", "get_pool_ohlcv", "get_bin_liquidity"]);
 ...
 */
// Role-specific token limits — smaller = faster + cheaper
const ROLE_MAX_TOKENS = {
  MANAGER:  600,   // just needs close/claim/hold + short reason
  SCREENER: 1024,  // needs to reason about pool selection
  GENERAL:  null,  // use config value (full responses for user chat)
};
    ...
    try {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        response = await client.chat.completions.create({
          ...
          max_tokens: maxOutputTokens ?? ROLE_MAX_TOKENS[agentType] ?? config.llm.maxTokens,
        });
